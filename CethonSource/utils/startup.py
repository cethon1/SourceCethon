...
from CethonSource import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    """
    To set up bot for CethonSource
    """
    try:
        await mody.connect()
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await mody.tgbot.send_file(
                ...
                caption="**‏۞︙ بــوت سيثون يـعـمـل بـنـجـاح ✓ \n۞︙ أرسل `.الاوامر`لرؤية اوامر السورس \n  ۞︙ لأستعمال بوت الأختراق عبر كود التيرمكس أرسل`.هاك`**",
                buttons=[(Button.url("سورس سيثون", "https://t.me/cethon"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    try:
        ...
        if starkbot.bot_inline_placeholder:
            ...
            try:
                await mody.send_message("@BotFather", "/setinline")
                await asyncio.sleep(1)
                await mody.send_message("@BotFather", botname)
                await asyncio.sleep(1)
                await mody.send_message("@BotFather", joker)
...
CethonSource = {"@cethon", "@ZeSupport"}
async def saves():
   for ELHYBA in CethonSource:
        try:
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"CethonSource/{folder}/*.py"
        plugin_path = f"CethonSource/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @cethon"
        photobt = await mody.upload_file(file="mody/razan/resources/start/ce1.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات سيثون ", mody, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await mody.upload_file(file="mody/razan/resources/start/ce2.JPEG")
        _, groupid = await create_supergroup(
    ...
    if flag:
        ...
        args = [executable, "-m", "CethonSource"]
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    await load_plugins(folder="CethonSource", extfolder=cfolder)
