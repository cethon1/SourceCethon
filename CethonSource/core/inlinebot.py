...
def main_menu():
    text = f"**- [سـورس سيثون](https://t.me/cethon)\
        \n المسـاعد\
        \n\
        \n◽  سيثون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("ced"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" CethonSource UsᴇʀBoᴛ", "https://t.me/cethon"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./CethonSource", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس  سيثون 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./CethonSource", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس  سيثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./CethonSource", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس  سيثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            _result = main_menu()
            result = builder.article(
                title="© CethonSource Help",
                description="Help menu for CethonSource",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/cethon"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/cethon1support",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت  سيثون 🧸♥](https://t.me/cethon)**\
            \n\
            \n❤  سيثون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/cethon1support)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@ced.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@ced.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@ced.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@ced.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
