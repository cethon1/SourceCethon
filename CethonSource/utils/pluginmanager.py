...
from CethonSource import CMD_HELP, LOAD_PLUG
...
LOGS = logging.getLogger("CethonSource")
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"CethonSource/plugins/{shortname}.py")
        checkplugins(path)
        name = "CethonSource.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"CethonSource/plugins/{shortname}.py")
            name = f"CethonSource.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["CethonSource.plugins." + shortname] = mod
        LOGS.info("۞︙تم بنجاح تحميل ملف ✓" + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"CethonSource/plugins/assistant/{shortname}.py")
        name = "CethonSource.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["CethonSource.plugins.assistant" + shortname] = mod
